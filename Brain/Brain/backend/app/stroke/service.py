...
def predict_brain_stroke(db:Session,patient,image:UploadFile):
    ...
    result=predict_stroke(str(image_path))
    patient_record={
...
def get_patient_image(db:Session,patient_id:str):
    ...
    if not image_path.exists():
        raise HTTPException(status_code=404,detail='Stroke MRI image not found.')
