...
from .service import predict_brain_tumor,delete_patient_service,get_patient_image
from . import crud

router=APIRouter(prefix='/tumor',tags=['Brain Tumor'])

@router.post('/predict',response_model=PredictionResponse,summary='Predict Brain Tumor')
async def predict(
    ...
    return predict_brain_tumor(db=db,patient=patient,image=image)
...
@router.get('/image/{patient_id}',summary='View patient MRI')
